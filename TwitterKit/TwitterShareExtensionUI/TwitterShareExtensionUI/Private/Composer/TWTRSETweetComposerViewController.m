...
- (CGRect)autoCompletionResultsViewControllerCalculatedFrame
{
    ...
    const CGRect frame = {
        .origin = { .x = 0 , .y = autoCompletionResultsVerticalOrigin },
        .size = { .width = _tableView.contentSize.width, .height = autoCompletionResultsHeight },
    };
    return frame;
}
...
- (instancetype)initWithConfiguration:(nonnull TWTRSETweetShareConfiguration *)configuration
{
    ...
    if ((self = [super initWithNibName:nil bundle:nil])) {
        ...
        if ([self isAutoCompletionAvailable]) {
            _autoCompletionResultsViewController = [[TWTRSEAutoCompletionTableViewController alloc] initWithAutoCompletion:configuration.autoCompletion
                                                                                                        imageDownloader:configuration.imageDownloader
                                                                                                               delegate:self];
        }
...
- (void)dealloc
{
    if (self.registeredForTweetTextViewContainerBoundsSizeKVO) {
        [self.tweetTextViewContainer removeObserver:self
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.titleView = [[UIImageView alloc] initWithImage:[TWTRSEUIBundle imageNamed:@"bird"]];
...
// TODO: drop-iOS-10: remove #if/#endif and all code within
        ...
        [self.tableView addObserver:self
                         forKeyPath:NSStringFromSelector(@selector(contentSize))
                            ...
                            context:TWTRSETableViewContentSizeKVOContext];
        self.registeredForTableViewContentSizeKVO = YES;

        [self.dataSource addObserver:self
                          forKeyPath:[NSString stringWithFormat:@"%@.%@", NSStringFromSelector(@selector(composedTweet)), NSStringFromSelector(@selector(text))]
                             options:0
                             context:TSETweetTextKVOCOntext];
        self.registeredForTweetTextKVO = YES;

        if ([self isAutoCompletionAvailable]) {
            [self.dataSource addObserver:self
                              forKeyPath:NSStringFromSelector(@selector(textSelection))
                                 options:NSKeyValueObservingOptionNew
                                 context:TWTRSEDataSourceCursorSelectionKVOCOntext];
...
- (void)_tseui_presentNoAccountsErrorAlert
{
    ...
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:[TSELocalized localizedString:TSEUI_LOCALIZABLE_SHARE_EXT_NO_ACCOUNTS_TITLE]
                                                                             message:[TSELocalized localizedString:noAccountsMessage]
...
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:[TSELocalized localizedString:TSEUI_LOCALIZABLE_SHARE_EXT_TWEET_FAILED_TITLE]
                                                                             message:[TSELocalized localizedString:TSEUI_LOCALIZABLE_COMPOSITION_SEND_TWEET_ERROR_LABEL]
                                                                      preferredStyle:UIAlertControllerStyleAlert];
...
- (void)_tseui_presentNoLocationPermissionAlert
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:[TSELocalized localizedString:TSEUI_LOCALIZABLE_LOCATION_SERVICES_ACCESS_DENIED_ALERT_TITLE]
                                                                             message:[TSELocalized localizedString:TSEUI_LOCALIZABLE_LOCATION_SERVICES_ACCESS_DENIED_ALERT_MESSAGE]
                                                                      preferredStyle:UIAlertControllerStyleAlert];
...
- (void)setAutoCompletionResultsVisible:(BOOL)visible
{
    ...
    if ([self isAutoCompletionResultsDisplayAllowed] && visible != _autoCompletionResultsVisible) {
        ...
        [UIView animateWithDuration:(visible) ? 0.4 : 0.2 delay:0.01 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_tseui_tweetButtonTapped
{
    ...
    [_configuration.networking sendTweet:[self.dataSource.composedTweet copy] fromAccount:self.selectedAccount completion:^(TWTRSENetworkingResult result) {
